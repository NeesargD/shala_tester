                                ...
                                state.quizModel.content[index].answers!.length > 5
                                    ? Padding(
                                        ...
                                        child: GridView.builder(
                                          gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 2, crossAxisSpacing: 23, mainAxisSpacing: 23, childAspectRatio: 4 / 3),
                                    ...
                                    : ListView.builder(
                                        ...
                                        itemCount: state.quizModel.content[index].answers!.length,
                                        itemBuilder: (context, i) {
                                          if (state.quizModel.content[index].questionId!.questionType == 'Multiple') {
                                          ...
                                          } else if (state.quizModel.content[index].questionId!.questionType == 'Singlechoice') {
                                            return listItemSingleChoice(state.quizModel.content[index], index, i);
                                          } else if (state.quizModel.content[index].questionId!.questionType == 'Date') {
                                            return listItemDate(state.quizModel.content[index], index, i);
                                          } else {
                                            return listItemMultipleChoice(state.quizModel.content[index].answers![i], state.quizModel.content[index].questionId!.questionType);
                                          }
                                        },
                        ...
                        Align(
                          ...
                          child: CustomButton(
                              onTap: () {
                                ...
                                if (state.quizModel.content[index].questionId!.questionType == 'Multiple') {
                                  state.quizModel.content[index].answers!.forEach((element) {
                                    if (element.isSelected) {
                                      selectedAnswers.add(element.id.toString());
                                    }
                                  });
                                } else if (state.quizModel.content[index].questionId!.questionType == 'Singlechoice') {
                                  selectedAnswers.add(state.quizModel.content[index].answers![state.quizModel.content[index].questionId!.selectedIndex!].id.toString());
                                } else if (state.quizModel.content[index].questionId!.questionType == 'Date') {
                                  selectedAnswers.add(state.quizModel.content[index].answers![0].answerValue.toString());
                                }

                                SubmittedAnswer submit = SubmittedAnswer(questionId: state.quizModel.content[index].questionId!.id, answer: selectedAnswers);
                              ...
                              buttonText: index == state.quizModel.content.length - 1 ? AppLocalizations.of(context)!.translate('done') : AppLocalizations.of(context)!.translate('next'),
  ...
  Widget listItemMultipleChoice(Answer answer, String questionType) {
    return InkWell(
      ...
      child: Container(
        ...
        child: Text(
          answer.answerValue,
          style: answer.isSelected ? TextStyles.R1875.copyWith(color: ColorRes.white) : TextStyles.R1875,
        ),
      ),
    );
  }

  Widget listItemSingleChoice(Content content, int questionIndex, int answerIndex) {
    return InkWell(
      onTap: () {
      ...
      child: Container(
        ...
        child: Text(
          ...
          style: content.questionId!.selectedIndex == answerIndex ? TextStyles.R1875.copyWith(color: ColorRes.white) : TextStyles.R1875,
        ),
      ),
    );
  }
